...
class YoloNode:
    def __init__(self):
        ...
        self.path1 = f'{pkg_path}/models/person.pt'
        self.path2 = f'{pkg_path}/models/traffic_light.pt'
        self.model1 = YOLO(self.path1)
        self.model2 = YOLO(self.path2)  # 경로를 올바르게 수정
    ...
    def compute_highest_collision_probability(self, predicted_boxes, frame):
        ...
        if self.mission_info.num == 7 :
            roi_x1 = int(width* 0.1)
            roi_x2 = int(width * 0.9)
            roi_y1 = int(height * 0.35 )
            roi_y2 = int(height * 0.9)
        else : 
            roi_x1, roi_y1, roi_x2, roi_y2 = self.set_roi_by_mission(self.mission_info.num, width, height)
    ...
    def mission_callback(self, msg):
        try:
            ...
            rospy.logerr(f"Invalid driving mode value: {self.mission_info.num}")
    
    #mission number 에 따라 다르게 동작하는 로직  z
    def timer_callback(self, event): 
        
        if self.mission_info.num in [5, 51, 7, 71]:#주차 때는 충돌확률 계산 하지 않기
            return

        if self.latest_frame is not None: # 이미지가 있을경우
            
            if self.mission_info.num in [3, 4, 6]:
                self.detect_traffic_light(self.latest_fram)
    ...
    def detect_traffic_light(self, frame):
        ...
        results = self.model2(tensor_frame)
        filtered_results = self.filter_results_by_confidence(results)
        for object_id, box, conf, cls in filtered_results:
            if cls == 0 or cls == 1:  # 초록이나 좌회전일 경우
                self.traffic_color = 3
            elif cls == 2:  # 노란색일 경우
                self.traffic_color = 2
            elif cls == 3:  # 빨간색일 경우
                self.traffic_color = 1
    ...
    def calculate_collision_probability(self, frame):
        try:
            # 이미지 전처리 및 GPU로 이동
            tensor_frame = self.preprocess_image(frame)
            results = self.model1(tensor_frame)
            filtered_results = self.filter_results_by_confidence(results)
